"""Centralized worker job lifecycle statuses for persistence and UI."""
...
def replay_stats_transition(
    ...
    *,
    ...
    After a successful **ok**, a later replay (``processing`` then terminal) should not bump
    ``processed``/``failed`` again. We set ``replay_stats_suppress`` when entering ``processing``
    from **ok**; the next terminal clears it without counting.

    After **error**, ``processing`` clears the latch so a genuine retry can count a new outcome.
